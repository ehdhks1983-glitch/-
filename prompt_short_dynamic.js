...
6. ★자연스러움이 규칙보다 우선: 명사형으로 줄이다 어색한 합성어가 생기면(예: "물일", "손보호컷") 명사형을 버리고 짧은 합쇼체 단정문으로 써라. "오래 끼는 물일"(✕) → "설거지가 길어져도 편합니다"(○).
7. ★일상에서 안 쓰는 단어 금지: 사전에 있어도 쇼핑몰에서 안 쓰는 말은 쓰지 마라("물일"→"설거지·물 닿는 집안일"). 출력 전에 소리 내어 읽어보고, 실제 쿠팡 상세페이지에서 본 적 있는 문장처럼 자연스러운지 자가검증하라. 어색하면 다시 써라.
8. ★구매욕구 공식 - 기능 서술로 끝내지 마라: 각 헤드라인은 아래 중 하나의 힘을 가져야 한다.
   (a) 혜택 선두: 고객이 얻는 것 먼저 — "설거지 끝나도 보송한 손"
   (b) 손실 회피: 안 사면 잃는 것 — "맨손 설거지가 손을 상하게 합니다"
   (c) 장면 환기: 그 순간을 떠올리게 — "고무장갑 벗을 때마다 축축했다면"
   "물일 내내 손 보호" 같은 기능 요약문(✕)은 사고 싶은 이유가 아니다.`;
...
→ 자가점검: 이 상품군의 주된 셀링포인트가 '확인된 숫자'와 함께 보이는가? 분위기 문장만 있으면 실패다.`;
  ...
  function validateCopyPlanV92(copyPlan, productName){
    ...
    // 8-a) v21.8.24.110: 표시광고법 위험 표현 — 과장·단정 광고 표현은 플랫폼 제재/법적 리스크라 무조건 잡는다.
    {
      const adLawRe = /(최저가|업계\s*1위|판매\s*1위|국내\s*1위|세계\s*최초|국내\s*최초|100\s*%\s*(효과|만족|차단|제거)|완치|부작용\s*(없음|제로|0)|무조건\s*(효과|개선)|영구적|평생\s*보장|전액\s*환불\s*보장|특허\s*받은\s*효능|의학적으로\s*증명)/;
      secs.forEach(s => {
        const all = [s.main, s.sub, ...s.cards.map(c => c.head + ' ' + c.desc)].join(' | ');
        const m = all.match(adLawRe);
        if(m) violations.push(`섹션${s.num}에 표시광고법 위험 표현("${m[0]}") — 플랫폼 제재 대상. 확인된 사실 기반 표현으로 교체(예: "최저가"→가격 표현 삭제, "100% 차단"→"빈틈을 줄입니다")`);
      });
    }
