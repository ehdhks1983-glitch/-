...
6) ★한 특징 쏠림 금지(매우 중요): 부차 특징 하나(걸이·색상·디자인·포장 등)나 같은 소재가 '3개 섹션 이상'에 반복되면 실패다.
   부차 특징은 '최대 1개 섹션'만. 주된 효과(우산=방풍·자동개폐 / 화장품=지속력·발림성 / 청소기=흡입력)가 반드시 섹션을 차지해야 한다.
   → 배정을 끝낸 뒤, 같은 단어가 3섹션 이상 등장하지 않는지 세어보고, 넘으면 다른 강점으로 다시 배정하라.
...
9. ★카피 구체성 - 분위기만 말하지 마라: "차분한 톤으로 보기 편해요", "은은해서 무난해요"처럼 느낌·분위기만 있는 문장은 금지(정보량 0).
...
────────────────────
쿠팡 판매 상위 상세페이지는 친근한 '~어요' 말투를 거의 쓰지 않는다. 어떤 상품군이든 아래로 통일하라.
1. 메인 카피(헤드라인)는 '명사형 단정'으로: "완벽한 발수"(우산), "24시간 지속 보습"(화장품), "흡입력 2배"(청소기), "구김 없는 마감"(의류). 문장 어미('~해요/~합니다')를 붙이지 마라.
2. 서브·본문은 합쇼체('~합니다/~ㅂ니다')로: "바람에도 안 뒤집힙니다", "세탁 후에도 형태를 유지합니다".
3. ★해요체 금지: '~어요/~아요/~해요/~좋아요/~어울려요/~네요' 같은 친근체를 쓰지 마라. (예: "덜 젖어요" → "덜 젖습니다", "촉촉해요" → "촉촉하게 마무리됩니다")
...
────────────────────
1. ★핵심 셀링포인트 먼저: '이 상품군'에서 고객이 돈을 내는 가장 강한 구매 이유 3~4개를 정하고 거기에 섹션을 몰아줘라.
   예) 우산=대형·방풍·자동개폐·방수 / 화장품=지속력·발림성·성분 / 청소기=흡입력·무게·소음 / 의류=핏·소재·세탁 후 변형.
   부차 특징(걸이·색상·포장 등)은 1섹션 이하.
2. ★확인된 대표 숫자/스펙을 후크로: "126cm", "24시간", "450g"처럼 사용자 입력/링크로 '확인된' 수치 하나를 메인 헤드라인으로 키워라.
   단, 여러 스펙을 줄줄이 나열하지 마라(한 섹션 = 한 숫자). 확인 안 된 수치·리뷰수·별점·판매량은 절대 지어내지 마라.
3. ★크기·강도·효과는 '비교'로 체감시켜라: "일반 ○○보다 ~", "기존 제품은 ~". 비교 프레임을 최소 1섹션 권장.
4. 사회적 증거(판매량·리뷰·1위 등)는 '확인된 경우에만'. 없으면 형태·사용 장면으로 대체하라.
→ 자가점검: 이 상품군의 주된 셀링포인트가 '확인된 숫자'와 함께 보이는가? 분위기 문장만 있으면 실패다.`;

  // v21.8.24.106: 자동제작 전 '기획안 5종' — ChatGPT 답변에서 기획안 블록을 파싱(전 카테고리 공용).
  function parseConceptOptionsV106(text){
    const t = String(text || '');
    const out = [];
    const blocks = t.split(/\n(?=\[?\s*기획안\s*\d)/).filter(b => /기획안\s*\d/.test(b.split('\n')[0] || ''));
    blocks.forEach(b => {
      const head = (b.split('\n')[0] || '');
      const num = parseInt((head.match(/기획안\s*(\d+)/) || [])[1] || '0', 10);
      const get = (labels) => {
        for(const l of labels){
          const m = b.match(new RegExp(l + '\\s*[:：]\\s*([^\\n/]+)'));
          if(m && m[1].trim()) return m[1].trim().replace(/\s{2,}/g,' ');
        }
        return '';
      };
      const name = get(['컨셉명','컨셉','이름']);
  ...
  function validateCopyPlanV92(copyPlan, productName){
    ...
    //    첫 화면은 '핵심 이득' 또는 '구매자 상황'으로 1초 안에 멈추게 해야 한다.
    {
      ...
      if(hero && hero.main){
        ...
        if(vapidEnding.test(m) || vapidPhrase.test(m))
          violations.push(`섹션${hero.num}(HERO) 메인 카피가 추상적 후크("${m}") — 첫 화면은 이 제품의 '핵심 이득 한 마디'나 '구매자가 겪는 그 순간 상황'으로 1초 안에 멈추게 하세요`);
      }
    }
    // 7-b) v21.8.24.100: 카피 구체성 — 느낌/분위기만 있고 정보량 0인 서브·카드 설명을 잡는다.
    //    분위기어 + 무른 마무리(편해요/좋아요/어울려요)인데 숫자·상황·비교 같은 '구체 앵커'가 하나도 없으면 위반.
    {
      const vibeWord = /차분|은은|무난|깔끔|세련|단정|심플|모던|우아|부드러|감각|분위기|색감|톤으로|톤이라|느낌/;
      const softEnd = /(편해요|편합니다|좋아요|좋습니다|어울려요|어울립니다|예뻐요|이뻐요|괜찮아요|멋져요|만족해요|무난해요|무난합니다|은은해요|깔끔해요)\s*$/;
      // 구체 앵커: 숫자/단위·기능어·상황어·비교어. (바 한글 단위 '분/일/초'는 '차분/일상'처럼 오탐나서 제외 — 숫자는 \d로 잡음)
      const concrete = /\d|cm|mm|kg|[0-9]+\s*k|%|방풍|발수|자동|이중|초경량|한\s*손|버튼|접(고|어|히)|펴(고|서|져)|뒤집|보다|처럼|만큼|대신|없이|않(아|게|니)|안\s|걸어|걸면|넣으면|들면|쓸\s*때|출근|등교|장마|가방에/;
      const vibeOnly = (t) => {
        const x = String(t || '').trim();
        if(!x || x.length > 26) return false;
        return vibeWord.test(x) && softEnd.test(x) && !concrete.test(x);
      };
      secs.forEach(s => {
        const targets = [['서브 카피', s.sub]];
        s.cards.forEach((c, i) => targets.push([`카드${i + 1} 설명`, c.desc]));
        targets.forEach(([label, t]) => {
          if(vibeOnly(t)) violations.push(`섹션${s.num} ${label}이 정보 없는 분위기 표현("${t}") — 이 제품의 확인된 숫자·실제 사용 상황·비교 중 하나를 넣어 구체화하세요`);
        });
      });
    }
    ...
    if(secs.length >= 4){
      ...
      if(top && topN >= threshold)
        violations.push(`'${top}~'가 ${topN}/${secs.length}개 섹션에 반복 — 한 특징/소재에 쏠렸습니다. 그 특징은 1개 섹션만 다루고, 나머지는 이 상품군의 '주된 효과(고객이 돈을 내는 핵심 기능)'를 최소 1섹션 포함해 서로 다른 강점으로 재배정하세요`);
    }
...
이미지 안 문구를 만들기 전에 한 번 소리 내어 읽었을 때 어색하면 반드시 다시 쓰세요.`;
  ...
  const SECTION_COPY_ROLES_90 = {
    ...
    GIFT:'선물받는 사람의 상황을 떠올리게 하세요. 처음 장만하는 사람, 매일 쓰는 사람처럼 받는 사람의 사용 장면 중심으로.',
  ...
  // v21.8.24.107: 잡화/의류 전략이 특정 상품(명함지갑/래쉬가드) 전용으로 굳어 있어, 다른 잡화·옷에도
  //  명함/물놀이 문구가 주입되던 문제 수정 — 카테고리 일반 전략으로 교체(실제 상품명 기준 구체화는 지시문으로 유도).
  const CATEGORY_COPY_STRATEGY_90 = {
    fashion_accessory:`[품목별 카피 전략 - 패션잡화(가방/지갑/벨트/모자/시계 등)]
핵심 고객: 디자인과 수납·휴대성이 자기 생활 장면에 맞는지 확인하고 잡화를 고르는 사람.
주요 고민: 실물 질감·크기가 사진과 다를까 / 수납이 실제로 편할까 / 매일 들기에 부담스럽지 않을까 / 선물로 줘도 괜찮을까.
구매 이유: 꺼내고 넣는 동선이 단순함 / 손과 가방 안에서 부담 없는 크기 / 옷차림에 자연스럽게 어울림 / 선물로 무난함.
권장 표현: 꺼내는 순간, 가방 속 자리 하나, 매일 들어도 부담 없이, 손에 잡히는 크기 — 이 상품의 실제 사용 순간(상품명 기준)으로 바꿔 구체화하세요.
주의: 첫인상, 단정함 같은 추상어를 직접 반복하지 말고 꺼내는 순간, 정돈된 형태처럼 장면으로 바꾸세요. 자석 내장 등 원본에서 확인 안 된 기능 금지.`,
    fashion_clothing:`[품목별 카피 전략 - 의류]
핵심 고객: 핏, 소재감, 사이즈가 나에게 맞을지 확인하고 옷을 고르는 사람.
주요 고민: 화면과 실물의 색·핏이 다를까 / 사이즈 선택 실패 / 소재가 불편하지 않을까 / 세탁·관리가 번거롭지 않을까.
구매 이유: 확인된 실루엣과 색상 / 활동이 편한 착용감 / 코디 고민이 줄어드는 구성 / 입을 상황(출근·외출·운동·휴가)에 맞는 활용.
권장 표현: 입는 순간의 실루엣, 하루 종일 편한 착용감, 고민 없이 손이 가는 옷 — 이 상품의 실제 착용 장면(상품명 기준)으로 바꿔 구체화하세요.
...
한글 문장 중간에 불필요한 영문 혼용 금지. 한글 오타 금지.`;
  ...
  function visualDnaV94(section){
    ...
    if(/COMPARE|VERSUS|VS\b|BEFORE/.test(s)) return { layout:'★비교 다이어그램(이 제품 vs 일반 제품 나란히, 차이 나는 지점에 콜아웃)', crop:'두 대상 나란히 또는 상하 분할', bg:'좌우/상하 톤을 살짝 다르게(차이 강조)', cards:'차이 콜아웃 라벨 1~2개', angle:'비교 정렬' };
  ...
  function modelDirectiveV76(section, analysis){
    ...
    if(/WEAR_SCENE|FIT|USAGE|HOW_TO_USE|ROUTINE|LIFESTYLE|USE_SCENE|MOOD|ROOM_MOOD/i.test(section))
      return `\n[모델] 제품을 실제로 ${verb}하는 자연스러운 한국인 모델을 함께(제품이 주인공, 모델은 맥락·손/동작 위주, 표정·손·비율 왜곡 없이 실제 촬영처럼). 모델이 든 제품도 첨부 원본과 똑같은 그 제품이어야 합니다(다른 디자인 대체 금지).`;
    if(/PROBLEM|PAIN/i.test(section))
      return `\n[모델] 그 불편을 겪는 상황의 한국인 모델을 자연스럽게(표정·동작으로 공감, 제품은 작게/맥락). 제품이 등장한다면 첨부 원본과 동일해야 합니다.`;
    if(section==='HERO' && /fashion_clothing|footwear|beauty|kids|fitness|sports/.test(cat))
      return `\n[모델] 제품을 ${verb}한 한국인 모델의 라이프스타일 히어로컷(제품이 또렷이 보이게, 얼굴·손 왜곡 없이). ${verb}한 제품은 첨부 원본과 똑같아야 합니다.`;
...
제품·배경·연출 비주얼에만 집중하고, 상단 약 28% 영역은 헤드라인이 들어갈 자리로 단순하게(여백/배경) 비워두세요. 작은 포인트 아이콘 외 문장형 글자 금지.`;
    ...
    // v21.8.24.104: 카피 내용이 '비교 프레임'(~보다/일반/기존)이면 섹션명과 무관하게 비교 다이어그램 비주얼로 전환.
    const copyAll = `${(parsed && parsed.main) || ''} ${(parsed && parsed.sub) || ''}`;
    const isCompareCopy = /보다\s|일반\s|기존\s|vs/i.test(copyAll) && !/HERO|CTA/i.test(String(section));
    const dna = isCompareCopy ? visualDnaV94('COMPARE') : visualDnaV94(section);
    const composeLine = `레이아웃: ${dna.layout} / 제품 크롭: ${dna.crop} / 배경: ${dna.bg} / 카드: ${dna.cards} / 앵글: ${dna.angle}.`;
    // v21.8.24.104: 메인 카피에 확인된 숫자(126cm·8K·24시간 등)가 있으면 그 숫자를 화면 최대 요소로(베스트셀러 패턴).
    const numHookLine = (parsed && parsed.main && /\d/.test(parsed.main))
      ? `\n★숫자 강조: 메인 카피의 숫자("${(parsed.main.match(/[\d.,]+\s*[a-zA-Z가-힣%]{0,4}/) || [''])[0].trim()}")를 화면에서 가장 큰 요소로, 포인트 컬러로 강조하세요(숫자가 후크).` : '';
    // v21.8.24.106: 연출 다양화 — 장면형 섹션이 매번 같은 상황(예: 명함지갑=미팅에서 건넴)으로 나오던 문제.
    // 실행 시드+섹션 위치로 '생활 상황 아키타입'을 회전시켜 만들 때마다·섹션마다 다른 장면이 나오게 한다.
    // 사용자가 기획안(컨셉)을 골랐다면 그 상황이 우선이므로 회전은 생략.
    const SCENE_POOL_V106 = ['아침 출근·등교 준비 장면','이동 중(대중교통·차 안·거리)','집 안 일상(거실·주방·현관)','직장·작업 공간','친구·가족·동료와 함께하는 자리','야외 외출·나들이','쓰고 난 뒤 정리·보관하는 순간','선물하거나 선물 받는 순간'];
    const isSceneish = /SCENE|USAGE|LIFESTYLE|PROBLEM|PAIN|SOLUTION|ROUTINE|WEAR|HOW_TO/i.test(String(section));
    let sceneVarietyLine = '';
    if(isSceneish){
      if(data.conceptLine){
        sceneVarietyLine = `\n상황 연출: 선택된 기획안의 '핵심 상황'을 중심으로 연출하세요(앞 섹션과 같은 장소·구도 반복은 피하기).`;
      } else {
        const pick = SCENE_POOL_V106[(((_runVariantSeed || 0) + (typeof idx === 'number' ? idx * 3 : 0)) >>> 0) % SCENE_POOL_V106.length];
        sceneVarietyLine = `\n상황 다양화: 이번 컷의 생활 장면은 '${pick}' 각도로 연출하세요(앞 섹션과 같은 장소·상황 반복 금지). 제품 용도와 자연스럽게 어울리는 선에서 응용하세요.`;
      }
    }
...
★제품 재현 기준(최우선 — 위반 시 실패): 이 이미지의 제품은 '첨부 원본 사진 속 그 제품'이어야 합니다.
- 형태·부품 개수·비율·색상·질감·라벨/로고 위치를 원본과 똑같이. 비슷한 다른 제품(다른 모양 손잡이, 다른 버클, 다른 스티치, 다른 색)으로 대체 금지.
- ★연출컷·모델컷·상황컷에서도 동일: 장면이 바뀌어도 제품 자체는 원본과 1:1로 같아야 합니다. 연출을 위해 제품 디자인을 바꾸지 마세요.
- 원본에서 안 보이는 면(뒷면·바닥 등)을 그릴 땐 보이는 면과 일관되게, 새로운 장식·부품을 만들어 붙이지 마세요.
금지: 상위노출·판매1위·인증·천연가죽·방수·임의 수치·수납매수·리뷰·별점·확인 안 된 성분/효능${data.showPrice ? '' : '·가격/금액/할인율/판매가'} 표현 금지.${catRule}

━━━━━━━━━━━━━━━━━━━━
[3. 구매 설득 기준]
━━━━━━━━━━━━━━━━━━━━
타겟: ${tTarget}${tPain ? `\n고객 고민: ${tPain}` : ''}${tBenefit ? `\n핵심 장점: ${tBenefit}` : ''}${data.conceptLine ? `\n★선택된 기획안(이 컨셉으로 연출): ${_clean(data.conceptLine, 180)}` : ''}
이 섹션에서 전달할 한 가지 메시지: "${secMsg}"
카피 톤: 헤드라인은 명사형 단정(예: "완벽한 발수"), 본문은 합쇼체("~합니다"). 해요체("~어요/~해요")·전문용어·추상어·과장어 금지${_tone ? `, ${_tone} 톤` : ''}.
...
목적: ${secPurpose}
구도: ${shotHint}.
시각 구조: ${composeLine}${numHookLine}${sceneVarietyLine}${(typeof idx === 'number' && idx > 0) ? `
...
[7. 최종 검수]
━━━━━━━━━━━━━━━━━━━━
1) ★생성한 제품을 첨부 원본과 나란히 비교했을 때 같은 제품인가? (형태·부품 수·색·라벨 위치 중 하나라도 다르면 실패 — 다시 그릴 것)  2) 원본에 없는 로고·인증·리뷰·별점·수치가 추가되지 않았는가?
...
비율: ${ratio}.`;
  ...
  function generateDynamicPrompts({data={}, inferred=null, sectionCount='8', masterBrief='', copyPlan='', refStyle='', heroLockFromIndex=1, designMood='auto'}={}){
    ...
    // v21.8.24.104: ★카피 기획서가 있으면 이미지 섹션 흐름을 '기획서의 섹션'과 1:1로 맞춘다.
    // 이전엔 내부 추천 흐름(OVERVIEW/BENEFIT 등)과 기획서 섹션명(USP/SCENE 등)이 어긋나
    // 검증까지 통과한 카피가 통째로 버려지고 ChatGPT가 즉흥 카피를 쓰는 문제가 있었다.
    let sections = pickSections(analysis, sectionCount);
    if(copyPlan){
      const planSecs = listPlanSectionsV92(copyPlan);
      if(planSecs.length >= 2){
        const roles = planSecs
          .sort((a, b) => (a.num || 0) - (b.num || 0))
          .map(p => String(p.role || '').toUpperCase().replace(/[^A-Z_/]/g, '').split('/')[0])
          .filter(Boolean);
        if(roles.length >= 2) sections = roles;
      }
    }
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━` : '';
  ...
  window.DP_DYNAMIC_PROMPTS = { generateDynamicPrompts, VARIATION_SEEDS, DESIGN_MOODS, DETAILPAGE_LAYOUT_TEMPLATES, COPY_QUALITY_RULES, COPY_QUALITY_RULES_SHORT, COPY_MESSAGE_MAP_RULES, COPY_SECTION_STRUCTURE_RULES, COPY_HOOK_RULES, COPY_TONE_RULES, COPY_BESTSELLER_RULES, validateCopyPlanV92, listPlanSectionsV92, parseConceptOptionsV106 };
})();
