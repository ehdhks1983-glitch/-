...
드래프트에 **공개 이미지 URL(https)** 을 넣으면 이미지 글로 발행됩니다. 발행 시 Meta가 이미지를 가져와 처리하는 동안 컨테이너 `status` 가 `FINISHED` 될 때까지 자동으로 대기(폴링)한 뒤 게시합니다. 처리 실패/시간초과는 큐에 “실패”로 표시되어 다시 시도할 수 있습니다.
