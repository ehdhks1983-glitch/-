...
> **현재 버전 v1.11.0** — 대본 의미에 따라 숫자·펀치·목록·비교·후기·검색·단계·CTA 8종 장면을 자동 배치합니다. 글꼴 조합 5종과 장면별 수동 선택을 지원하며, 편집 속도는 `화면+말소리`·`말소리만`·`영상 화면만`으로 나눠 적용할 수 있습니다.

v1.11 핵심:

- 영상 길이에 따라 텍스트 장면 수 자동 조절(적게/자동/풍부하게)
- 정보·쇼핑·리뷰·홍보 콘텐츠별 장면 우선순위
- 자막 검토 화면에서 줄마다 장면 자동/일반/8종 직접 선택
- 기본·정보·임팩트·친근·레트로·UGC 글꼴 조합
- 말소리만 배속할 때 자막 시각도 함께 보정
- 영상 화면만 배속할 때 내레이션을 자르지 않고 원래 영상 길이 유지
...
- **🖥 긴 영상 (가로 16:9)** — 풀영상 하나를 구간별로 나누고 전체 내레이션 한 트랙으로 완성
- **🛒 쇼핑 상품 영상** — 상품 링크·사진·설명으로 홍보 영상 제작
- **🔗 블로그 글로 만들기** — 블로그 글의 사진·대본을 가져와 영상 제작
