        ...
        parts: list = [
            {
                "text": (
                    f"{prompt}\n\n"
                    + ("세로형(9:16) 유튜브 쇼츠" if canvas.h >= canvas.w
                       else "가로형(16:9) 유튜브")
                    + " 배경 이미지. 글자 없이, 하단 1/3은 자막이 올라갈 수 있게 단순하게."
                )
            }
        ]
...
IMAGE_STYLES = {
    ...
    # v0.74 — 그림체 다양화 (사용자 요청)
    "웹툰": "한국 웹툰 스타일, 셀 셰이딩과 선명한 윤곽선, 생동감 있는 색과 감정 표현, 만화적 구도",
    "지브리풍": "지브리풍 애니메이션 배경, 따뜻한 손그림 질감과 서정적인 자연광, 포근하고 향수 어린 분위기",
    "유화": "유화 회화, 두꺼운 붓 터치와 캔버스 질감, 풍부하고 깊은 색감, 고전 회화의 무게감",
    "동화": "파스텔 동화책 삽화, 부드러운 색연필·크레용 질감, 포근하고 따뜻한 그림책 분위기",
    "흑백": "흑백 모노톤, 강한 명암 대비와 감각적인 그레이스케일, 미니멀하고 세련된 무드",
    "픽셀아트": "레트로 픽셀 아트, 또렷한 도트 그래픽과 제한된 팔레트, 8비트 게임 감성",
}
